...
export default function PublicShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 overflow-x-hidden px-4 py-5 transition-all duration-300 min-[390px]:px-5 sm:gap-8 sm:px-6 sm:py-8 lg:px-8 lg:py-12 2xl:max-w-6xl">
      {/* dir="ltr" keeps brand physically left; language toggle does not swap layout */}
      <nav
        className="flex min-w-0 flex-col items-start gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between"
        dir="ltr"
      >
        <Link
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center justify-between gap-2 sm:w-auto sm:justify-end sm:gap-3">
          <div className="flex min-w-0 max-w-full rounded-2xl border border-white/10 bg-slate-950/85 p-1 text-xs font-semibold shadow-sm sm:bg-slate-950/60">
            {NAV_LINKS.map((link) => {
              ...
              return (
                <Link
                  className={`min-h-11 rounded-xl px-2.5 py-2 transition focus:outline-none focus:ring-2 focus:ring-cyan-300/50 min-[390px]:px-3 ${
                    active

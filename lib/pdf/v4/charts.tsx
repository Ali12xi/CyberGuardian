...
import { bf } from "@/lib/pdf/v4/components";
import type { LocaleV4 } from "@/lib/pdf/v4/components";
import { CONTENT_W, dark } from "@/lib/pdf/v4/constants";
...
export function GaugeChartV4({
  ...
  const scoreWord = locale === "ar" ? "النتيجة" : "Score";
  return (
    <View style={{ width: 100, height: 100, alignItems: "center" }}>
      ...
      <Text style={[{ fontSize: 18, fontWeight: 700, color: dark.textPrimary, marginTop: -14 }, bf(locale)]}>
        {lrmScoreText(`${labelPct}%`)}
      </Text>
      <Text style={[{ fontSize: 8, color: dark.textSecondary, marginTop: 2 }, bf(locale)]}>{scoreWord}</Text>
    </View>
  );
}

export type DonutCountsV4 = { high: number; medium: number; low: number };

export function DonutChartV4({ counts, locale }: { counts: DonutCountsV4; locale: LocaleV4 }) {
  ...
  const labHigh = locale === "ar" ? "مرتفع" : "High";
  const labMed = locale === "ar" ? "متوسط" : "Medium";
  const labLow = locale === "ar" ? "منخفض" : "Low";
  ...
  if (total <= 0) {
    return (
      <View>
        ...
        <Text style={[{ fontSize: 7, color: dark.textMuted, marginTop: 4, textAlign: "center" }, bf(locale)]}>
          {locale === "ar" ? "لا توجد نتائج مصنّفة" : "No classified findings"}
        </Text>
      </View>
    );
  }
  ...
  const segs: { n: number; color: string; label: string }[] = [
    { n: high, color: dark.red, label: labHigh },
    { n: medium, color: dark.amber, label: labMed },
    { n: low, color: dark.green, label: labLow },
  ];
    ...
    <View>
      ...
      <View style={{ marginTop: 4 }}>
        {segs
          .filter((x) => x.n > 0)
          .map((seg, i) => (
            <Text key={i} style={[{ fontSize: 7, color: dark.textSecondary }, bf(locale)]}>
              {`${seg.label}: ${lrmScoreText(String(seg.n))}`}
            </Text>
          ))}
      </View>
    </View>
...
export function ScoreBreakdownBarsV4({
  rows,
  locale,
}: {
        ...
        return (
          <View key={i} style={{ marginBottom: 6 }} wrap>
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 2 }}>
              <Text style={[{ fontSize: 8, color: dark.accentCyan }, bf(locale)]}>{r.label}</Text>
              <Text style={[{ fontSize: 8, color: r.color }, bf(locale)]}>
                {lrmScoreText(`${r.score}/${r.maxScore}`)}
              </Text>
            </View>
...
export function AttackSurfaceBarsV4({
  bars,
  locale,
}: {
        ...
        return (
          <View key={i} style={{ marginBottom: i === bars.length - 1 ? 0 : 10 }} wrap>
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
              <Text style={[{ fontSize: 8, color: dark.accentCyan }, bf(locale)]}>{b.label}</Text>
              <Text style={[{ fontSize: 8, color: b.color }, bf(locale)]}>{lrmScoreText(b.value)}</Text>
            </View>
            <View style={{ height: 4, width: barW, backgroundColor: dark.arcTrack, borderRadius: 2, overflow: "hidden" }}>
